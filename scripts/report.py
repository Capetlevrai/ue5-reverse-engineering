...
def build_report(out,profile):
    ...
    diagnostics=read_json(raw/'scan-diagnostics.json') if (raw/'scan-diagnostics.json').exists() else []
    errors=[]
    project={}
    ...
    for f in files:
        ...
        if '/Plugins/' in path and '/Content/' in path:indexed_roots[path.split('/Content/',1)[0]]+=1
    ...
    scripts=[x for x in installation if PurePosixPath(x['path']).suffix.lower()=='.as']
    script_folders=Counter('/'.join(x['path'].split('/')[:3]) for x in scripts)
    ...
    indexed_components=[{'path':k,'files':v,'evidence':'content paths in a readable directory index; no descriptor or runtime activation inferred'} for k,v in sorted(indexed_roots.items())]
    ...
    status='blocked-format' if not files and diagnostics else 'blocked-encrypted-indexes' if not files and encrypted else 'partial' if diagnostics or missing or metadata_failures or errors or any(r['status']!='parsed' for r in registries) else 'indexed'
    summary={
        ...
        'status':status,'scanDiagnostics':len(diagnostics),'indexedPluginContentRoots':len(indexed_components),'looseScriptFiles':len(scripts),'looseScriptBytes':sum(x['bytes'] for x in scripts),'containerFiles':dict(Counter(PurePosixPath(x['path']).suffix.lower() for x in installation if x['path'].lower().startswith(profile.get('paks',profile['project']+'/Content/Paks').lower().rstrip('/')+'/') and PurePosixPath(x['path']).suffix.lower() in ('.pak','.utoc','.ucas'))),
    ...
    for filename,data in [('indexed-plugin-content.json',indexed_components),('loose-scripts.json',scripts),('loose-script-folders.json',dict(script_folders.most_common())),('summary.json',summary),('plugins.json',plugins),('content-folders.json',folders),('maps.json',maps),('blueprints.json',blueprints),('classes.json',dict(classes.most_common())),('project-plugin-declarations.json',list(declarations.values())),('loose-components.json',loose),('loose-binaries.json',binaries),('metadata-parse-errors.json',errors),('module-references.json',{k:sorted(v) for k,v in native_refs.items()})]:save_json(out/filename,data)
    ...
    if diagnostics:notes.append(f"{len(diagnostics)} erreurs de lecture/enregistrement : couverture incomplète ; voir raw/scan-diagnostics.json et parser.log.")
    ...
    md+=f"- {len(scripts):,} scripts AngelScript (.as) livrés hors archives ; {sum(x['bytes'] for x in scripts):,} octets. Seuls les chemins et tailles sont catalogués.\n\n"
